...
description: 按备考剩余时间选强度：面前 1 小时（数据/Agent/量化极简清单）、考前 3 天分科表格、考前 7 天与学习路线联动（手写 SQL/画数据流/口述因子链条）；末尾附高频清单速链，全部指向站内知识页与习题。
---
# 考前冲刺

按「时间剩多少」选强度；每条都链接到本站具体页面，便于开浏览器直接背。

## 面前 1 小时（极简）

**数据分析 / 商分 / 数科**

- 过一遍 [SQL 知识](/knowledge/01-data-infrastructure/sql/overview) 里的「常见追问」
- 打开 [SQL 习题](/interviews/01-data-analytics/sql/sql-exercises) 快速看 2～3 道窗口函数 / 留存类题思路
- 扫 [A/B 知识](/knowledge/03-statistics-experiment/ab-testing/ab-testing) 与 [A/B 习题](/interviews/01-data-analytics/business-warehouse/ab-exercises) 各读「流程 + 样本量 + 护栏指标」
...
在 [学习路线总览](./roadmap) 的三条主线各走一轮「知识页 → 对应面试题页」，额外要求：

- **数据向**：至少完整手写或口述 5 道 SQL（留存、窗口、漏斗各覆盖）
- **Agent**：用自己的项目画一张「检索 + 工具 + 记忆」数据流
- **量化**：口述一次「因子 IC → 回测净值 → 过拟合与样本外」链条
