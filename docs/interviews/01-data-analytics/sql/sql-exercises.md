...
- 若你希望按学习节奏递进训练，先看 [`SQL 7 天专项训练`](/knowledge/01-data-infrastructure/sql/08-sql-7day-camp)。
...
#### 6. 行转列题：按 `income_type` 展开为多个金额字段
...
#### 9. 电商 SQL 题：计算 `2023` 年 `618` 期间每个品类 `GMV` 同比增长率，取 `Top3`
...
#### 10. 电商 SQL 题：找出连续 `3` 天下单且日均消费超过 `100` 元的用户
...
#### 15. 海外区域运营 SQL 题：每天每个国家收入前十的司机及其工作时长
...
#### 16. SQL 基础追问：`left join` 和 `inner join` 的区别是什么？如何找重复值？
...
如果面试官追问“为什么用了 `left join` 后数据变多”，通常要想到右表连接键不唯一，导致左表一行匹配到右表多行。
...
#### 17. 内容平台 SQL 题：计算连续三天发布且播放量破万的创作者数量
...
#### 18. 旅游平台 SQL 题：统计 2025 年第一季度各目的地预订表现
...
#### 20. 汽车换电 SQL 题：找出每个月换电次数排名前十的用户
...
#### 21. 内容平台 SQL 题：每个月每个城市登录天数 Top3 用户
...
#### 22. 游戏业务 SQL 题：近一个月每周至少登录三次且每次游戏时长超过 60 分钟的活跃用户数
...
#### 23. SQL 基础题：用 `user_id` 和日期两个条件关联两张表
...
#### 24. SQL 长宽表转换题：使用 `lateral view explode(split())` 拆分字符串
...
#### 25. SQL 基础概念题：Map、JSON、临时表、Hive、`union` 和 `union all`
...
#### 26. 登录 SQL 题：用户登录次数前五；多天多表如何每天取前五？
...
#### 27. 留存 SQL 题：七日留存率怎么算？多天每天的留存怎么算？
...
#### 28. 内容创作者 SQL 题：连续 10 天发布指定类型视频的创作者
...
#### 30. 内容消费 SQL 题：快手垂类活跃、作者播放时长 Top100、连续活跃 7 天
...
#### 31. DAU SQL 题：累计活跃十天以上和连续活跃十天以上用户
...
#### 32. 订单 SQL 题：每月超时订单比例大于 0.1 的城市，以及连续 3 个月满足条件的城市
...
#### 34. SQL 追问：`left join` 的条件写在 `on` 和 `where` 里有什么区别？
...
#### 35. SQL 窗口函数题：从订单表中取最近 7 天都下单过的用户
...
#### 36. SQL 大数据追问：非常大的数据集如何去重？有脏数据和数据倾斜怎么办？
...
#### 39. 电商 SQL 题：三月份有消费行为用户的人均 GMV 及分布分箱
